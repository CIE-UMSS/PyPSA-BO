...
def geothermal_capacity_constraint(n):
    ...
    rhs = 100 #n.generators.loc[geothermal_i,'p_nom'].sum() #+ 880 #Maximum Geothermal potential MW in Bolivia
...
def CCGT_capacity_constraint(n):
    ...
    rhs = 1771 #1775 #125 #CCGT max potential MW based on EnergyScope limitation (378 first run and 359 fifth run) 
...
def OCGT_capacity_constraint(n):
    ...
    rhs = 452 #OCGT max potential MW based on EnergyScope limitation (64 fifth run)
...
def Oil_capacity_constraint(n):
    ...
    rhs = 83 #Diesel max potential MW based on EnergyScope limitation (625 first run and 577 fifth run)
...
def biomass_capacity_constraint(n):
    ...
    rhs = 269 #Biomass potential MW based on EnergyScope limitation (757 first run and 1134 fifth run) 
...
def Wind_capacity_constraint(n):
    ...
    rhs = 1998 #Onshore wind max potential MW based on EnergyScope limitation (6632 first run and 2106 fifth run) 
...
def Solar_capacity_constraint(n):
    ...
    rhs = 1827 #Solar max potential MW based on EnergyScope limitation (7352 first run and 9454 fifth run) 
...
def biomass_operation_constraint(n):
    ...
    biomass_limit = 425400 #MWh potential resources for biomass-based generation (1402940 first run) (5606002 fifth run)
    rhs = biomass_limit
...
def OCGT_operation_constraint(n):
    ...
    OCGT_limit = 22 #MWh potential resources for gas-based generation (25143 first run) (22958 fifth run)
    rhs = OCGT_limit
...
def CCGT_operation_constraint(n):
    ...
    CCGT_limit = 35 #MWh potential resources for gas-based generation (156512 first run) (160344 fifth run)
    rhs = CCGT_limit
...
def oil_operation_constraint(n):
    ...
    oil_limit = 4 #MWh potential resources for gas-based generation (19078 first run) (26758 fifth run)
    rhs = oil_limit
...
def battery_operation_constraint(n):
    ...
    battery_limit = 11026 #MWh potential resources for battery storage based on EVs (8016 for 1st run) (8071 second run)
    rhs = battery_limit
    define_constraints(n, lhs, '>=', rhs, 'Store', 'total_battery_capacity')
...
def load_sheeding_constraint(n):
    ...
    LS_limit = 1000000 #MWh potential resources for gas-based generation (19078 first run) (26758 fifth run)
    rhs = LS_limit
    define_constraints(n, lhs, '<=', rhs, 'Generator', 'load_shedding_generation')

#Capacity exapansion constraint for transmission lines and links
def transmission_lines_expansion_constraint(n):
    ext_lines_i = n.lines[n.lines.s_nom_extendable].index
    s_nom_current_lines = get_var(n, "Line", "s_nom")[ext_lines_i] # get_var calls the LP variable to be optimised (s_nom), e.g. equivalent to s_nom_opt (s_nom_opt is not defined as a variable but is the result of the optimisation)
    lhs = linexpr((1, s_nom_current_lines)).sum()
    rhs = (1 + 50/100) * n.lines.loc[ext_lines_i, "s_nom_min"].sum()  # existing recalculated thermal capacity (s_nom_min) + (50)% expansion cap
    define_constraints(n, lhs, "<=", rhs, "Line", "transmission_lines_expansion_limit")

# def per_line_transmission_lines_expansion_constraint(n):
#     ext_lines_i = n.lines[n.lines.s_nom_extendable].index
#     s_nom_vars = get_var(n, "Line", "s_nom")[ext_lines_i]
#     base = n.lines.loc[ext_lines_i, "s_nom_min"]  
#     # One constraint per line
#     for line in ext_lines_i:
#         lhs = linexpr((1, s_nom_vars[line]))
#         rhs = (1 + 100/100) * base[line] # existing + (100)% expansion cap per line
#         define_constraints(n, lhs, "<=", rhs, "Line", f"expansion_limit_{line}")

# def transmission_links_expansion_constraint(n):
#     ext_links_i = n.links[n.links.p_nom_extendable].index
#     p_nom_current_links = get_var(n, "Link", "p_nom")[ext_links_i]
#     lhs = linexpr((1, p_nom_current_links)).sum()
#     rhs = n.links.loc[ext_links_i, "p_nom"].sum() + n.lines["s_nom"].sum()  # existing + base transmission capacity (kinda arbitrary but is required for testing) 
#     define_constraints(n, lhs, "<=", rhs, "Link", "transmission_links_expansion_limit")
...
def extra_functionality(n, snapshots):
    ...
    ext_lines_i = n.lines[n.lines.s_nom_extendable].index
    if not ext_lines_i.empty:
        transmission_lines_expansion_constraint(n)
        #per_line_transmission_lines_expansion_constraint(n)
    # ext_links_i = n.links[n.links.p_nom_extendable].index
    # if not ext_links_i.empty:
    #     transmission_links_expansion_constraint(n)
